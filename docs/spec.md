...
- 検索対象チェックボックス: 徒歩/在来線の計算対象を切替。片方のみ選択で該当ルートのみ計算。
...
## ルート計算仕様
- Google Directions API でチェック対象の徒歩/WALKING と在来線/TRANSIT を計算。
...
- 在来線は開始/終了地点の最寄り駅を使って検索し、駅までの徒歩時間を推定して合算する。
- 目標時間の調整:
  - 許容差: 目標時間の±`max(8, round(targetMinutes * 0.1))` 分。
...
- 駅検索クエリ上限: 12件。
- 散歩ルート再検索回数: 最大4回。
- 時間許容差: 目標時間の±`max(8, round(targetMinutes * 0.1))` 分。
