      ...
      readJson(req)
        .then(async (payload) => {
          ...
          if (!responseSent) {
            ...
            const adjustment =
              payload?.adjustment === "longer" ||
              payload?.adjustment === "shorter"
                ? payload.adjustment
                : "";
            ...
            if (estimatedDistanceKm && adjustment === "longer") {
              distanceMinKm = Math.max(
                1,
                Math.round(estimatedDistanceKm * 1.1 * 10) / 10
              );
              distanceMaxKm =
                Math.round(estimatedDistanceKm * 1.4 * 10) / 10;
            } else if (estimatedDistanceKm && adjustment === "shorter") {
              distanceMinKm = Math.max(
                1,
                Math.round(estimatedDistanceKm * 0.6 * 10) / 10
              );
              distanceMaxKm =
                Math.round(estimatedDistanceKm * 0.9 * 10) / 10;
            }
            const originRegion =
              ...
              if (stops.length < 2) {
                ...
                sendOnce(200, {
                  place: {
                    ...
                    target_minutes:
                      Number.isFinite(parsedTargetMinutes) &&
